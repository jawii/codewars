...
func step(_ g: Int, _ m: Int, _ n: Int) -> (Int, Int)? {
	...
	var startPrime = startNumber.findClosestPrimeToUp()

	while startPrime <= endNumber {
		if (startPrime + askedStep).isPrime {
			return (startPrime, startPrime + askedStep)
		}
		
		startPrime = (startPrime + 1).findClosestPrimeToUp()
	}
...
//
//	for prime in primesBetweenNumbers {
//		if let askedPrime = (primesBetweenNumbers.filter { $0 == prime + askedStep }).first {
//			return (prime, askedPrime)
//		}
//	}
